package org.elkd.core.raft.election
...
class ElectionScheduler private constructor(private val voteRequest: RequestVoteRequest,
  ...
  fun schedule() {
    ...
    /* Vote for self */
    electionTally.recordUpVote(clusterMessenger.clusterSet.selfNode.id)
  ...
  private fun dispatchVoteRequest() {
    ...
    clusterMessenger.clusterSet.nodes.forEach {
      ...
      future.addListener(Runnable {
        handleVoteResponse(it, future.get())
      }, executor)
    }
  }

  private fun handleVoteResponse(node: Node, response: RequestVoteResponse) {
    try {
      when (response.isVoteGranted) {
