...
class ClientService(private val commandRequestHandler: CommandRequestHandler) : ElkdClientServiceGrpc.ElkdClientServiceImplBase() {
  init {
  ...
  override fun clientCommand(request: RpcClientCommandRequest?, responseObserver: StreamObserver<RpcClientCommandResponse>?) {
    Pools.clientCommandThreadPool.execute {
      commandRequestHandler.handle(request!!, responseObserver!!)
    }
  }
